...
class AlbumSongsViewController: HTCollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        albumThumbImage.setImageWithUrlRequest(NSURLRequest(URL: url), placeHolderImage: nil, success: { (request, response, image, fromCache) -> Void in
            self.albumThumbImage.image = image
            self.albumThumbImage.contentMode = .ScaleToFill
            }) { (request, response, error) -> Void in
                self.albumThumbImage.image = noImage
        }
...
extension AlbumSongsViewController: UICollectionViewDataSource {
